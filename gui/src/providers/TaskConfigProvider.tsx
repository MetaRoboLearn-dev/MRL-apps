...
export const TaskConfigProvider = ({
  ust,
  task,
  mode,
  children,
  ...
  const [isLogged, setIsLogged] = useState<boolean>(
    ust?.activity_task.is_logged || false,
  );
  const [hasRobotAccess, setHasRobotAccess] = useState<boolean>(
    ust?.activity_task.allows_robot || false,
  );
  ...
  const [textures, setTextures] = useState<Record<Sticker, Texture>>(
    {} as Record<Sticker, Texture>,
  );
  const [barrierTextures, setBarrierTextures] = useState<
    Record<Barrier, Texture>
  >({} as Record<Barrier, Texture>);
  ...
  const rotateBy90 = () => {
    ...
    if (new_rot >= 360) setSelectedRotation(0);
    else setSelectedRotation(new_rot);
  };
  ...
  const loadTextures = () => {
    ...
    const taskStickerValues = new Set<string>(
      (task.stickers || [])
        ...
        .map(
          (s) =>
            (Sticker[s.sticker as keyof typeof Sticker] as
              | string
              | undefined) ?? s.sticker,
        ),
    );
    ...
    const entries = Object.entries(Stickers).filter(([key]) =>
      taskStickerValues.has(key),
    );
      ...
      entries.forEach(([key, sticker]) => {
        loader.load(sticker.image, (texture) => {
          ...
          if (loadedCount === entries.length) {
            setTextures(
              (prev) =>
                ({ ...prev, ...textureMap }) as Record<Sticker, Texture>,
            );
          }
        });
      });
  ...
  const loadPackTextures = (packName: string) => {
    ...
    fetch(`/api/stickers/pack/${encodeURIComponent(packName)}`, {
      credentials: "include",
    })
      .then((r) => r.json())
      ...
      .catch((e) =>
        console.error(`Failed to load textures for pack "${packName}"`, e),
      );
  };
